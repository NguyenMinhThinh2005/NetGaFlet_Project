...
// Import hook useIsFocused để kiểm tra xem màn hình hiện tại có đang được hiển thị hay không
...
export function useShake(onShake: () => void, threshold = 1.8) {
  // Sử dụng hook useIsFocused để xác định trạng thái hiển thị của màn hình hiện tại
  ...
  useEffect(() => {
    // Nếu màn hình không được hiển thị (đang ở màn hình khác), không kích hoạt cảm biến
    if (!isFocused) return;
        ...
        subscription = Accelerometer.addListener(data => {
          ...
          if (lastX.current !== null && lastY.current !== null && lastZ.current !== null) {
            ...
            if (dx + dy + dz > threshold) {
              ...
              onShake();
              // Hẹn giờ sau 2 giây (2000ms) sẽ mở khóa cờ cooldown cho phép nhận tín hiệu rung tiếp theo
              setTimeout(() => {
    ...
    // Thực hiện đăng ký lắng nghe
    subscribe();

    // Dọn dẹp: Hủy đăng ký lắng nghe cảm biến khi component chứa hook bị hủy bỏ (unmounted)
    return () => {
      if (subscription) {
        subscription.remove();
      }
    };
  }, [onShake, threshold, isFocused]); // Chạy lại hiệu ứng nếu hàm callback, ngưỡng hoặc trạng thái focus thay đổi
}
